...
const borrowsSlice = createSlice({
  ...
  reducers: {
    ...
    updateBorrow: (state, action) => {
      const { id, bookId, borrowerName, borrowerPhone, borrowerEmail, borrowDate, dueDate } = action.payload;
      const updatedState = state.map((borrow) =>
        borrow.id === id
          ? { ...borrow, bookId, borrowerName, borrowerPhone, borrowerEmail, borrowDate, dueDate }
          : borrow
      );
      saveBorrowsToLocalStorage(updatedState);
      return updatedState;
...
export const { addBorrow, deleteBorrow, updateBorrow } = borrowsSlice.actions;
