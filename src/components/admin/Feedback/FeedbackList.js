...
const FeedbackList = () => {
  ...
  const handleResponseSubmit = async (id, response) => {
    setIsLoading(true);
    try {
      await feedbackService.respondToFeedback(id, { response }); // Dùng `response` thay vì `message`

      // Cập nhật lại trạng thái của phản hồi sau khi đã trả lời
  ...
  return (
    <div className="p-4">
      ...
      {/* Modal để hiển thị câu trả lời đã gửi */}
